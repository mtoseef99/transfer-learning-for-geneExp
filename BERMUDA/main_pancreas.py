...
from BERMUDA import training, testing, epochs_list, loss_total_list, loss_reconstruct_list, loss_transfer_list
...
# IMPORTANT PARAMETER
similarity_thr = 0.8 #S_thr in the paper, choose between 0.85-0.9
print("similarity_thr: ", similarity_thr)
...
num_epochs = 2000
base_lr = 0.01
lr_step = 50  # step decay of learning rates
momentum = 0.9
l2_decay = 5e-5
...
if __name__ == '__main__':
    ...
    ################################################################
    ####  Saving results to a file for current experiment ##########
    ################################################################

    # a list for all losses with corrosponding epoch
    final_all_loss_pancreas = pd.DataFrame({'Epoch': epochs_list,
        'Total loss': loss_total_list,
        'Reconstruct loss': loss_reconstruct_list,
        'Transfer loss': loss_transfer_list
    })


    # change the path to working directory
    f_name = 'Results/Final_results_pancreas_Model2.xlsx'
    final_all_loss_pancreas.to_excel(f_name)
...
print("--- %s seconds ---" % (time.time() - start_time))
    ...
    #     final_all_loss_pancreas.to_excel(res_path + str(time.time()) + f_name)
    # else:
    #     final_all_loss_pancreas.to_excel(f_name)

    # evaluate_scores_ae = pd.DataFrame()
    # evaluate_scores_uncorrected = pd.DataFrame()